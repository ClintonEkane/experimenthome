...
// HiveMQ Cloud credentials — replace with your actual broker details
...
class _ExperimentScreenState extends State<ExperimentScreen> {
  ...
  Future<void> _tryClaimStation() async {
    final available = widget.stations.where((s) => s.isAvailable).firstOrNull;
    if (available == null) return;

    final user = _auth.currentUser!;
    await _firestore.lockStation(
      experimentId: widget.experiment.id,
      stationId: available.id,
      userId: user.uid,
      displayName: user.displayName ?? user.email ?? 'Unknown',
    );
    setState(() => _myStation = available);
    await _connectMqtt(available.id);
  }
  ...
  void _resetIdleTimer() {
    ...
    _idleTimer = Timer(_idleTimeout, () {
      if (_sessionActive) _stopSession();
    });
  }
      ...
      child: Column(
        ...
        children: [
          ...
          if (_sessionActive) ...[
            const SizedBox(height: 8),
            Text(
              'Auto-stops after 5 min of inactivity',
              textAlign: TextAlign.center,
